...
namespace mopmc::queries {
    ...
    void GpuConvexQuery<T, I>::query() {
        ...
        Vector<T> vt = Vector<T>::Zero(m, 1);
        Vector<T> vb = Vector<T>::Zero(m, 1);
        Vector<T> *vPtr;
        std::vector<T> r1;
        Vector<T> r;
        Vector<T> w(m);
        ...
        while (iter < maxIter && (Phi.size() < 3 || fDiff > eps)) {
            ...
            fDiff = std::abs(fn.value(vt) - fn.value(vb));
        ...
        std::cout << "*Threshold approximation* - upper bound: " << fn.value(vt) << ", lower bound: " << fn.value(vb)
                  <<
                  ", gap: " << fDiff << "\n";
