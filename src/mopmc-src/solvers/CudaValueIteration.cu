    ...
    namespace value_iteration {
        namespace gpu {
            ...
            int CudaValueIterationHandler<ValueType>::initialise() {
                ...
                CHECK_CUDA(cudaMalloc((void **) &dRi, A_nrows * sizeof(double)))
                // cudaMemcpy -------------------------------------------------------------
            ...
            int CudaValueIterationHandler<ValueType>::valueIterationPhaseOne(const std::vector<double> &w) {
                ...
                do {
                    ...
                    //mopmc::functions::cuda::maxValueLauncher2(dY, dX, dRowGroupIndices, dPi, dPi_bin, A_ncols + 1);
            ...
            int CudaValueIterationHandler<ValueType>::valueIterationPhaseTwo() {
                ...
                CHECK_CUDA(cudaMalloc((void **) &dMasking, A_nnz * sizeof(int)))
                ...
                }
                {
                    ...
                    CHECK_CUDA(cudaMemcpy(hMasking.data(), dMasking, k * sizeof(int), cudaMemcpyDeviceToHost))
                    printf("____ dMasking: [");
                    for (int i = 0; i < k; ++i) {
                        std::cout << hMasking[i] << " ";
                    }
                    std::cout << "...]\n";
                ...
                thrust::copy_if(thrust::device, dA_values, dA_values + A_nnz - 1,
                                dMasking, dB_values, mopmc::functions::cuda::is_not_zero<int>());
                thrust::copy_if(thrust::device, dA_columns, dA_columns + A_nnz - 1,
                                dMasking, dB_columns, mopmc::functions::cuda::is_not_zero<int>());
                thrust::copy_if(thrust::device, dA_rows_extra, dA_rows_extra + A_nnz - 1,
                                dMasking, dB_rows_extra, mopmc::functions::cuda::is_not_zero<int>());
